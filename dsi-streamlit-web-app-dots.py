...
credit_score = st.number_input(
    label = "01. Enter the customer's cedit score",
    min_value = 0,
    max_value = 1000,
    value = 500
    )
...
if st.button("Submit for Prediction"):
    ...
    # store our input data in a dataframe for prediction
    new_data = pd.DataFrame({"age" : [age], "gender" : [gender], "credit_score" : [credit_score]})
    
    # apply input dataframe to the model pipeline and extract probability prediction
